...
| `POST /api/worktrees/{id}/remove` | The checkout as well as the workspace. `force` is only sent after Herdr has refused a checkout with uncommitted work in it and the phone has asked a second time. |
